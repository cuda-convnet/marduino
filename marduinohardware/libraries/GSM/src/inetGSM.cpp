...
const char* USER_AGENT = "User-Agent: HAAT-Hardware";

int InetGSM::httpGET(const char* server, int port, const char* path, char* result, int resultlength, char* token = "", boolean isBasic = false)
{
     ...
     */
     ...
     if(strcmp(token, "") !=0 ){
		...
		if(isBasic) {
			gsm.SimpleWrite(" Basic ");
		}else{
			gsm.SimpleWrite(" Bearer ");
		}
		gsm.SimpleWrite(token);
		gsm.SimpleWrite("\r\n");
	  }
     gsm.SimpleWrite(USER_AGENT);
     gsm.SimpleWrite("\r\n\r\n");
...
int InetGSM::httpPOST(const char* server, int port, const char* path, const char* parameters, char* result, int resultlength, char* token = "", boolean isBasic = false)
{
     boolean connected=false;
     int n_of_at=0;
     char itoaBuffer[8];
